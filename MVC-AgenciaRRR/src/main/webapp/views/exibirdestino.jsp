...
<body class="container-fluid bd-navbar">
	<main class="bd-navbar">
		...
		<section class="py-2 ml-0 mr-0">	
			<h1>Destinos Cadastrado</h1>	
			<div class="card-body bd-navbar">
			<form action="../criar-destino" class="form-control bd-navbar">
				<table class="table table-striped bd-navbar">
					...
					<tbody>
						<jstl:forEach items="${listaDestinos}" var="destinos">
							<tr>
								<td>${destinos.nomeDestino}</td>
								<td>${destinos.categoriaDestino}</td>
								<td>${destinos.condicao}</td>
								<td>${destinos.precoUnit}</td>
								<td>${destinos.qtdDisponivel}</td>
								<td>${destinos.dataIda}</td>
								<td>${destinos.horaIda}</td>
								<td>${destinos.dataChegada}</td>
								<td>${destinos.horaChegada}</td>
								<td><a href="editar-destino?idDestino=${destinos.idDestino}"
									class="btn btn-success">Editar</a> <a
									href="excluir-destino?idDestino=${destinos.idDestino}"
