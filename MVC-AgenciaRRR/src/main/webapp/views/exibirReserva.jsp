	...
	<main class="bd-navbar">
		...
		<div class="conteiner">
			<div class="row justify-content-center">
					...
					<div class="card-body bd-navbar">
						<form action="../criar-reserva" class="form-control bd-navbar">
							<table class="table table-striped bd-navbar">
								<thead>
									<tr>
										<th>Nome Cliente</th>
										<th>Forma Pagamento</th>
										<th>Destino Reservado</th>
										<th>Categoria</th>
										<th>Condicao</th>
										<th>Valor total</th>
										<th>Qt. Reservada</th>
								...
								<tbody>
									<jstl:forEach items="${listaReservas}" var="reservas">
										<tr>
											<td>${reservas.cliente.nomeCliente}</td>
											<td>${reservas.pagamento}</td>
											<td>${reservas.destino.nomeDestino}</td>
											<td>${reservas.destino.categoriaDestino}</td>
											<td>${reservas.destino.condicao}</td>
											<td>${reservas.precoTotal}</td>
											<td>${reservas.quantReservada}</td>
											<td>${reservas.destino.dataIda}</td>
											<td>${reservas.destino.horaIda}</td>
											<td>${reservas.destino.dataChegada}</td>
											<td>${reservas.destino.horaChegada}</td>
											<td>${reservas.statusPedido}</td>
											<td>
												<a href="editar-reserva?idReserva=${reservas.idReserva}"
												class="btn btn-success">Editar</a> 
												<a href="excluir-reserva?idReserva=${reservas.idReserva}"
		...
		<div class="container-fluid">
			<footer class="py-6 my-4">
				<ul
					class="nav justify-content-center border-bottom navbar navbar-expand-lg">
